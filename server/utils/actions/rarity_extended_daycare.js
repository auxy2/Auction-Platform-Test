...
export async function careOfAll(
  { provider, tokensID },
  onError,
  onSuccess = onSuccessToast
) {
  const [hadToApprove, success] = await approveRarityExtendedCare({
    provider,
    steps: 2,
  });
  if (!success) {
...
export async function careAdventure(
  { provider, tokensID },
  onError,
  onSuccess = onSuccessToast
) {
...
export async function careCellar(
  { provider, tokensID },
  onError,
  onSuccess = onSuccessToast
) {
  ...
  if (hadToApprove) {
    _toast = toast.loading(
      '2/2 - Sending adventurers in the Cellar adventurers...'
    );
  } else {
...
export async function careLevelup(
  { provider, tokensID },
  onError,
  onSuccess = onSuccessToast
) {
...
export async function careGold(
  { provider, tokensID },
  onError,
  onSuccess = onSuccessToast
) {

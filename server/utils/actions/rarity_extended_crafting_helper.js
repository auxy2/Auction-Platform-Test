...
export async function craft(
  { provider, tokenID, itemName, baseType, itemType, craftingMaterials },
  callback
) {
  let _toast;
  const signer = provider.getSigner();
  ...
  const isApproved = await isApprovedForAll({
    provider,
    toApprove: process.env.RARITY_CRAFTING_HELPER_ADDR,
  });
  if (!isApproved) {
    const raritySource = new ethers.Contract(
      process.env.RARITY_ADDR,
...
export async function cook(
  { provider, tokenID, mealAddress, itemName },
  callback
) {
  let _toast;
  const signer = provider.getSigner();
  const rarityCook = new ethers.Contract(
    ...
    [
      {
        inputs: [
          { internalType: 'address', name: '_meal', type: 'address' },
          { internalType: 'uint256', name: '_adventurer', type: 'uint256' },
  ...
  const isApproved = await isApprovedForAll({
    provider,
    toApprove: process.env.RARITY_COOKING_HELPER_ADDR,
  });
  if (!isApproved) {
    const raritySource = new ethers.Contract(
      process.env.RARITY_ADDR,
   ...
   **********************************************************************/
  try {
    const transaction = await rarityCook.cook(mealAddress, tokenID, tokenID);
    const transactionResult = await transaction.wait(process.env.DEFAULT_WAIT);
    if (transactionResult.status === 1) {
      callback({ error: false, data: tokenID });
      toast.dismiss(_toast);

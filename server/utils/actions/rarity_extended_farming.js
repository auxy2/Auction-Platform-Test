...
export async function levelup(
  { provider, tokenID, farmID, jobName },
  callback
) {
  let _toast;
  const signer = provider.getSigner();
...
export async function unlock(
  { provider, tokenID, farm, farmName, farmCost },
  callback
) {
  let _toast;
  let currentStep = 1;
    ...
    try {
      ...
      if (allowance.gte(element.amount)) {
        ...
        _toast = toast.loading(
          `${currentStep++}/${++steps} - Approving ${element.name}...`
        );
        const transaction = await IERC20.approve(
          tokenID,
          manager,
          element.amount
        );
  ...
  _toast = toast.loading(
    `${currentStep++}/${steps} - Unlocking ${farmName}...`
  );
  try {

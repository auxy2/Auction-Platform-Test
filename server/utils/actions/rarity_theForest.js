...
export async function exploreTheForest(
  { provider, tokenID, timeInDays },
  callback
) {
...
export async function discoverTreasureTheForest(
  { provider, tokenID },
  callback
) {
...
export async function levelUpTreasureTheForest(
  { provider, tokenID, adventurerID, treasureName, xpRequired },
  callback
) {
  let _toast = toast.loading(
    `1/3 - Approving Proxy Spender ${adventurerID}...`
  );
  const signer = provider.getSigner();
  ...
  try {
    _toast = toast.loading(
      `2/3 - Approving ${ethers.utils.formatEther(xpRequired)} XP to be used...`
    );
    ...
    const hasAllowance = await rarityXpProxy.allowance(
      address,
      adventurerID,
      process.env.DUNGEON_THE_FOREST_ADDR
    );
    if (hasAllowance.gte(xpRequired)) {
      ...
      const transaction = await rarityXpProxy.approve(
        adventurerID,
        process.env.DUNGEON_THE_FOREST_ADDR,
        xpRequired
      );
...
export async function restoreTreasureTheForest(
  { provider, tokenID, treasureName, adventurerID },
  callback
) {
  ...
  try {
    ...
    if (approvedAddr === process.env.DUNGEON_THE_FOREST_ADDR) {
      ...
      const transaction = await raritySource.approve(
        process.env.DUNGEON_THE_FOREST_ADDR,
        tokenID
      );
      const transactionResult = await transaction.wait(

...
const RARITY_MANIFEST = new ethers.Contract(process.env.RARITY_ADDR, [
...
export async function equip(
  { provider, tokenID, minter, itemID, itemName, slot },
  callback
) {
  let _toast;
  let currentStep = 1;
  ...
  try {
    ...
    if (approved !== process.env.RARITY_EQUIPMENT_WRAPPER_ADDR) {
      _toast = toast.loading(
        `${currentStep++}/${++steps} - Approving ${itemName}...`
      );
      const transaction = await minterContract.approve(
        process.env.RARITY_EQUIPMENT_WRAPPER_ADDR,
        itemID
      );
      const transactionResult = await transaction.wait(
  ...
  _toast = toast.loading(
    `${currentStep}/${steps} - Trying to equip ${itemName}...`
  );
  try {
...
export async function rEquip(
  ...
  try {
    const transaction = await rarityEquipmentContract.set_rEquipement(
      slot,
      tokenID,
      tokenID,
      minter,
...
export async function buyBasicSet(
  { provider, tokenID, setID, setName },
  callback
) {
  let _toast;
  const signer = provider.getSigner();
  ...
  try {
    ...
    const transaction = await rarityEquipmentBasicSet.buySet(setID, tokenID, {

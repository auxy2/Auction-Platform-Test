...
export async function approveERC721(
  { provider, contractAddress, spender, tokenID, name },
  callback
) {
...
export async function approveERC20(
  { provider, contractAddress, adventurerID, spender, amount, name },
  callback
) {
  let _toast = toast.loading(`Approving ${name}...`);
  const signer = provider.getSigner();
...
export async function isERC20ApprovedUint({
  provider,
  contractAddress,
  adventurerID,
  spender,
...
export async function approveAdventurer(
  { provider, adventurerID, name },
  callback
) {
  let _toast = toast.loading(`Approving ${name}...`);
  const signer = provider.getSigner();
...
export async function isAdventurerApproved({
  provider,
  adventurerID,
  spender,
}) {

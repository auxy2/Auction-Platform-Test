...
export const PlaceBid = () => {
  ...
  const [currentValue, setCurrentValue] = useState(
    initialStoredValue !== null ? JSON.parse(initialStoredValue) : null
  );
  ...
  const handlePlaceBid = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:9000/api/products/${productId}/place-bid`,
        {
          method: 'POST',
          headers: {
    ...
    >
      ...
      >
        <Paper elevation={5} sx={{ paddingX: 6, paddingY: 5 }}>
          ...
          <Typography mb={1} mt={3} variant="body1">
            Minimum Amount to bid is ${minBidAmount}
          </Typography>
          <Typography mb={1} mt={3} variant="body1">
            {' '}
      ...
      {isWarningOpen && (
        ...
        >
          ...
          Bid amount must be a number and at least{' '}
          <strong>${minBidAmount}</strong>.
        </Alert>
      )}
        ...
        >
          <FormControl>
            ...
            <Typography variant="body1">
              Congratulations! Your bid of ${bidAmount} has been placed.
            </Typography>
            <Button
              variant="contained"
              color="primary"
